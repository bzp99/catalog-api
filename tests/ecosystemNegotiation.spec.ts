...
import { testProvider2,testProvider3,testOrchestrator,testConsumer1,testConsumer2 } from "./fixtures/testAccount";
import {
  sampleDataResource,
...
before(async () => {
  ...
  const serverInstance = await startServer(3008);
...
mockContract();
  ...
  // Login provider1
  const provider1AuthResponse = await request(app).post("/v1/auth/login").send({
    email: testProvider2.email,
    password: testProvider2.password,
  });
  provider1Jwt = provider1AuthResponse.body.token;
    // Login provider2
    const provider2AuthResponse = await request(app).post("/v1/auth/login").send({
...
describe("Ecosystem routes tests", () => {
  ...
  it("should configure Participant Ecosystem Offerings", async () => {
    ...
    modifiedSampleOfferings.offerings[0].serviceOffering = providerServiceOffering1Id;
    const response = await request(app)
  ...
  it("should create join request by a service provider", async () => {
    const modifiedSampleJoinRequest = { ...sampleJoinRequest };
    modifiedSampleJoinRequest.offerings[0].serviceOffering = consumerServiceOffering2Id;
    const response = await request(app)
      .post(`/v1/ecosystems/${ecosystemId}/requests`)
      .set("Authorization", `Bearer ${consumer2Jwt}`)
      .send(modifiedSampleJoinRequest)
